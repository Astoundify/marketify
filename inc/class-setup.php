...
class Jobify_Setup {
    ...
    public function __construct() {
		...
		$this->steps = array(
			...
			'setup-widgets' => array(
				...
				'completed' => is_active_sidebar( 'widget-area-front-page' ),
				'description' => 
					'<p>' . __( 'Manage your widgets to control what displays on your homepage, listing pages, standard pages, and more.', 'jobify' ). '</p>' .
					'<p>' . sprintf( '<a href="%s" class="button button-primary button-large">%s</a>', admin_url( 'widgets.php' ), __( 'Manage Widgets', 'jobify' ) ) . '</p>',
				'documentation' => array(
					'Widget Areas' => 'http://listify.astoundify.com/category/352-widget-areas',
					'Widgets' => 'http://listify.astoundify.com/category/199-widgets' 
				)
			...
			'support-us' => array(
				...
				'description' => __( 'Help improve Jobify by submitting a rating and helping to translate the theme to as many languages as possible!', 'jobify' ),
				'completed' => 'na',
				'documentation' => array(
    ...
    public function add_page() {
		add_submenu_page( 'themes.php', __( 'Jobify Setup', 'jobify' ), __( 'Setup Guide', 'jobify' ), 'manage_options', 'jobify-setup', array( $this, 'output' ) );
    }
    ...
    public function add_meta_boxes() {
		foreach ( $this->steps as $step => $info ) {
			add_meta_box( $step , $info[ 'title' ], array( $this, 'step_box' ), 'jobify_setup_steps', 'normal', 'high', $info );
		}
    }
    ...
    ?>
		<h1><?php printf( __( 'Welcome to Jobify %s', 'jobify' ), $this->theme->Version ); ?></h1>
		...
		<div class="jobify-badge"><img src="<?php echo get_template_directory_uri(); ?>/images/listify-banner-welcome.jpg" width="140" alt="" /></div>
    <?php
    }
    ...
    ?>
		<p class="helpful-links">
			<a href="http://jobify.astoundify.com" class="button button-primary"><?php _e( 'Documentation', 'jobify' ); ?></a>&nbsp;
    ...
    public function steps() {
		do_accordion_sections( 'jobify_setup_steps', 'normal', null );
    }
}
