	...
	<section class="<?php echo ! is_active_sidebar( 'sidebar-download-single-comments' ) || ! is_singular( 'download' ) ? 'col-xs-12' : 'col-sm-8 col-xs-12'; ?>">
		...
		<?php if ( have_comments() ) : ?>
			...
			<nav id="comment-nav-below" class="comment-navigation" role="navigation">
				...
				<div class="nav-previous"><?php previous_comments_link( '<i class="icon-left-open-mini"></i> <span class="nav-title">' . __( 'Older Comments', 'marketify' ) . '</span>' ); ?></div>
				<div class="nav-next"><?php next_comments_link( '<span class="nav-title">' . __( 'Newer Comments', 'marketify' ) . '</span> <i class="icon-right-open-mini"></i>' ); ?></div>
